# -*- coding: utf-8 -*-
"""Kopia notatnika Oct 7 - Session 1 - Data Generation.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1tb2CYEBTyG_YFHfz3KTUUJ7UZao_Y90L
...
instr.simulate()
instr.write('basic-simulation.h5',mode = 'w')
...
print('Sampling rate:', instr.fs, 'Hz')
print('Duration:', instr.duration, 's')
print('Central frequency:', instr.central_freq, 'Hz')
# print(...)
...
instr.plot_fluctuations(skip=300)
instr.plot_offsets(skip=300)
instr.plot_totals(skip=300)
...
data = Data.from_instrument(instr)
...
ax[0].plot(instr.t[skip:], X[skip:], label="TDI X")
ax[1].plot(instr.t[skip:], Y[skip:], label="TDI Y")
ax[2].plot(instr.t[skip:], Z[skip:], label="TDI Z")
...
plt.figure()
plt.loglog(f, np.sqrt(psd_X2))
plt.loglog(f, np.sqrt(psd_Y2))
plt.loglog(f, np.sqrt(psd_Z2))
plt.xlabel('Frequency [Hz]')
plt.ylabel('ASD [/sqrt(Hz)]')
# plt.xlim([1e-4, 3e-2]) # restrict frequency band
plt.grid()
...
instr = Instrument(size = simlen,
                   lock = 'six',
                   orbits = 'instrument/tests/data/keplerian-orbits-2-0.h5')
...
instr.write('simulation-noise.h5', mode = 'w')
...
LISAnoisepsd = LISAnoisemodel.psd(f,option="X", tdi2=True,equal_arms=True)
...
noisedata = Data.from_instrument('simulation-noise.h5')

X_noise = X2.build(**noisedata.args)(noisedata.measurements) / instr.central_freq
Y_noise = Y2.build(**noisedata.args)(noisedata.measurements) / instr.central_freq
Z_noise = Z2.build(**noisedata.args)(noisedata.measurements) / instr.central_freq

skip = 300

fig, ax = plt.subplots(3,1, figsize = (12,9), sharex = True)
ax[0].plot(instr.t[skip:], X_noise[skip:], label="TDI X")
ax[1].plot(instr.t[skip:], Y_noise[skip:], label="TDI Y")
ax[2].plot(instr.t[skip:], Z_noise[skip:], label="TDI Z")
ax[2].set_xlabel("Time [s]")
ax[0].set_ylabel("TDI"), ax[1].set_ylabel("TDI"), ax[2].set_ylabel("TDI")
ax[0].legend(), ax[1].legend(), ax[2].legend()

f, psd_X2_noise = scipy.signal.welch(X_noise[skip:],
                               fs=instr.fs,
                               nperseg=len(X_noise[skip:]))
f, psd_Y2_noise = scipy.signal.welch(Y_noise[skip:],
                               fs=instr.fs,
                               nperseg=len(Y_noise[skip:]))
f, psd_Z2_noise = scipy.signal.welch(Z_noise[skip:],
                               fs=instr.fs,
                               nperseg=len(Z_noise[skip:]))

plt.figure()
plt.loglog(f, np.sqrt(psd_X2_noise), label = 'X noise')
plt.loglog(f, np.sqrt(psd_Y2_noise), label = 'Y noise')
plt.loglog(f, np.sqrt(psd_Z2_noise), label = 'Z noise')
plt.loglog(f, np.sqrt(LISAnoisepsd), 'k--', label= 'LISA noise model')
plt.xlabel('Frequency [Hz]')
plt.ylabel('ASD [/sqrt(Hz)]')
plt.xlim([1e-4, 1]) # restrict frequency band
plt.ylim([1e-26, 1e-16]) # restrict frequency band
plt.grid()
plt.legend()
...
t_min = 0
t_max = pMBHB["CoalescenceTime"]+1000
dt = 0.25
t = np.arange(t_min, t_max, dt)

hp, hc = hphc.compute_hphc_td(t) # takes 4Gb memory during ~40sec
...
ax[0].plot(t[-simlen:], hp[-simlen:])
ax[0].set_ylabel(r"$h_+$")
ax[1].plot(t[-simlen:], hc[-simlen:])
...
strain = ReadStrain(t_sim,
                    hp[-simlen:],
                    hc[-simlen:],
                    gw_beta=pMBHB["EclipticLatitude"],
                    gw_lambda=pMBHB["EclipticLongitude"],
                    orbits = 'instrument/tests/data/keplerian-orbits-2-0.h5')
...
instr.write('simulation-noise-gws.h5', mode = 'w')
...
simdata = Data.from_instrument('simulation-noise-gws.h5')

X = X2.build(**simdata.args)(simdata.measurements) / instr.central_freq
Y = Y2.build(**simdata.args)(simdata.measurements) / instr.central_freq
Z = Z2.build(**simdata.args)(simdata.measurements) / instr.central_freq
...
gwdata = Data.from_gws('gw.h5', 'instrument/tests/data/keplerian-orbits-2-0.h5')

X_gw = X2.build(**gwdata.args)(gwdata.measurements)
Y_gw = Y2.build(**gwdata.args)(gwdata.measurements)
Z_gw = Z2.build(**gwdata.args)(gwdata.measurements)

skip = 300

fig, ax = plt.subplots(3,1, figsize = (12,9), sharex = True)
ax[0].plot(instr.t[skip:], X_gw[skip:], label="TDI X")
ax[1].plot(instr.t[skip:], Y_gw[skip:], label="TDI Y")
ax[2].plot(instr.t[skip:], Z_gw[skip:], label="TDI Z")
ax[2].set_xlabel("Time [s]")
ax[0].set_ylabel("TDI"), ax[1].set_ylabel("TDI"), ax[2].set_ylabel("TDI")
ax[0].legend(), ax[1].legend(), ax[2].legend()
...
plt.figure()
plt.plot(instr.t[skip:], X_sum[skip:], label="noise + signal")
plt.plot(instr.t[skip:], X_noise[skip:], label="noise")
plt.plot(instr.t[skip:], X_gw[skip:], label="signal")
plt.xlabel("Time [s]")
plt.ylabel("TDI X")
plt.legend()
plt.grid()
...
f, psd_X2 = scipy.signal.welch(X[skip:],
                               fs=instr.fs,
                               nperseg=len(X[skip:]))
f, psd_X2_gw = scipy.signal.welch(X_gw[skip:],
                               fs=instr.fs,
                               nperseg=len(X_gw[skip:]))
f, psd_X2_sum = scipy.signal.welch(X_sum[skip:],
                               fs=instr.fs,
                               nperseg=len(X_sum[skip:]))

plt.figure()
plt.loglog(f, np.sqrt(psd_X2_noise), label = 'noise')
plt.loglog(f, np.sqrt(psd_X2_sum), label = 'sum of noise + MBHB signal TDIs')
plt.loglog(f, np.sqrt(psd_X2), label = 'complete simulation')

plt.loglog(f, np.sqrt(psd_X2_gw), ls = '-.', label = 'MBHB signal')
plt.loglog(f, np.sqrt(LISAnoisepsd), 'k--', label= 'LISA noise model')
plt.xlabel('Frequency [Hz]')
plt.ylabel(r"ASD of TDI $X$ [/sqrt(Hz)]")
plt.xlim([1e-4, 1]) # restrict frequency band
plt.ylim([1e-26, 1e-16]) # restrict frequency band
plt.grid()
plt.legend()
...
hdf5.create_dataset('Y', data=Y)
hdf5.create_dataset('Z', data=Z)
# Closing file
hdf5.close()
...
hdf5.create_dataset('X', data=X_gw)
hdf5.create_dataset('Y', data=Y_gw)
hdf5.create_dataset('Z', data=Z_gw)
# Closing file
hdf5.close()